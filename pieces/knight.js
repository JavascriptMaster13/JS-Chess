class Knight{
  ...
  legalMoves(board){
    ...
    for(let i = 0;i<possibleMoves.length;i++){
      const [posRow,posCol] = possibleMoves[i];
      if(row+posRow < board.length && row+posRow>=0 && col+posCol < board.length && col+posCol>=0){ //checking the boundries
        if(board[row+posRow][col+posCol]){
          if(board[row+posRow][col+posCol].color!==this.color){
